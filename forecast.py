...
def run_forecast(
    ...
    for step in range(nh + 1):
        ...
        # Blend prediction with current baseline using FORECAST_PREDICTION_WEIGHT (20% prediction power)
        if df is not None and len(df) > 0:
            current_pm = float(df["pm25"].mean())
            if prediction:
                predicted_pm = pm25_at_hour(prediction, float(step + 1))
                # 20% prediction + 80% current baseline
                target_pm = (config.FORECAST_PREDICTION_WEIGHT * predicted_pm + 
                            (1 - config.FORECAST_PREDICTION_WEIGHT) * current_pm)
            else:
                target_pm = current_pm
